...
class_files = {'order':[],'scan':[], 'name':[],'att':[], 'coll_m':[], 'lambda_nm':[],
               'detX_m':[], 'detY_m':[], 'temp_C':[], 'moni':[], 'time_s':[], 'name_hdf':[ ]}               
...
#for ii in range(0,10):
    ...
    class_files['coll_m'].append(round(float(np.asarray(file_hdf['/entry1/SANS/collimator/length'])), 1))
    class_files['time_s'].append(round(float(np.asarray(file_hdf['/entry1/SANS/detector/counting_time'])), 2))
    class_files['moni'].append(round(float(np.asarray(file_hdf['/entry1/SANS/detector/preset'])), 2))
    class_files['temp_C'].append(round(float(np.asarray(file_hdf['/entry1/sample/temperature'])), 2))
    class_files['detX_m'].append(round(float(np.asarray(file_hdf['/entry1/SANS/detector/x_position']))/1000, 1)) # in m
    class_files['detY_m'].append(round(float(np.asarray(file_hdf['/entry1/SANS/detector/y_position']))/1000, 2)) # in m
    name = str(np.asarray(file_hdf['/entry1/sample/name']))
    class_files['name'].append(name[3:-2])
    class_files['lambda_nm'].append(round(float(np.asarray(file_hdf['/entry1/data1/lambda'])), 2))
# save list of files    
...
for ii in range(0, len(class_files['att'])):
    if class_files['att'][ii] > 0 and class_files['detX_m'][ii] == trans_dist:
...
for jj in unique_det:
    ...
    path_det = os.path.join(path_dir_an, 'detX_' + string +'/')
    ...
    print(data)
    save_file = os.path.join(path_det, 'detX_files_'+ string + 'm.txt')
    with open(save_file, 'w') as f:
        with redirect_stdout(f):
            print(data)
    save_file = os.path.join(path_det, 'detX_files_'+ string + 'm.json')
    a_file = open(save_file, "w")
